...
import { resolveBashAction, resolveGlobAction, resolveExactAction, globMatch, isSubsequence } from "../src/matching.ts";

function cmd(name: string, args: string[]) {
  const raw = [name, ...args].join(" ");
  const commands = extractAllCommandsFromAST(parseBash(raw), raw);
  return { name: getCommandName(commands[0]!), args: getCommandArgs(commands[0]!) };
}
...
test("isSubsequence", async (t) => {
  await t.test("matches when all needles appear in order", () => {
    ...
    assert.equal(isSubsequence(["git", "status"], ["git", "status", "--short"]), true);
    assert.equal(isSubsequence(["git", "status"], ["git", "-v", "status"]), true);
  });
...
test("globMatch", async (t) => {
  ...
  await t.test("handles special regex characters", () => {
    ...
    assert.equal(globMatch("node_modules/.package-lock.json", "node_modules/.package-lock.json"), true);
  });
});
...
test("resolveBashAction", async (t) => {
  await t.test("allows base command when in rules", () => {
    ...
    assert.equal(resolveBashAction("git", ["commit", "-m", "msg"], rules), "allow");
    assert.equal(resolveBashAction("git", [], rules), "allow");
  });

  await t.test("allows specific subcommand", () => {
    assert.equal(resolveBashAction("git", ["status"], { "git status": "allow" }), "allow");
  });

  await t.test("allows subcommand with extra trailing args", () => {
    assert.equal(resolveBashAction("git", ["status", "--short"], { "git status": "allow" }), "allow");
    assert.equal(resolveBashAction("jira", ["issue", "view", "XXX-123"], { "jira issue view": "allow" }), "allow");
  });

  await t.test("allows subcommand with extra flags interspersed", () => {
    assert.equal(resolveBashAction("git", ["branch", "-v", "--show-current"], { "git branch --show-current": "allow" }), "allow");
  });

  await t.test("returns undefined for other subcommands when only specific one is allowed", () => {
    const rules = { "git status": "allow" as const };
    assert.equal(resolveBashAction("git", ["commit", "-m", "msg"], rules), undefined);
    assert.equal(resolveBashAction("git", [], rules), undefined);
  });

  await t.test("returns undefined when required tokens are missing", () => {
    assert.equal(resolveBashAction("git", ["branch", "-D", "main"], { "git branch --show-current": "allow" }), undefined);
  });

  await t.test("returns undefined for unknown commands", () => {
    assert.equal(resolveBashAction("curl", ["evil.com"], { "ls": "allow", "cat": "allow" }), undefined);
  });
  ...
  await t.test("* matches any command", () => {
    assert.equal(resolveBashAction("curl", ["evil.com"], { "*": "allow" }), "allow");
    assert.equal(resolveBashAction("rm", ["-rf", "/"], { "*": "allow" }), "allow");
  });
  ...
  await t.test("multiple subcommands can be allowed independently", () => {
    ...
    assert.equal(resolveBashAction("git", ["status"], rules), "allow");
    assert.equal(resolveBashAction("git", ["log", "--oneline"], rules), "allow");
  ...
  await t.test("multi-level subcommand matching", () => {
    ...
    assert.equal(resolveBashAction("jira", ["issue", "view", "PROJ-123"], rules), "allow");
    assert.equal(resolveBashAction("jira", ["issue", "list", "--project", "PROJ"], rules), "allow");
    assert.equal(resolveBashAction("jira", ["issue", "create"], rules), undefined);
    assert.equal(resolveBashAction("jira", ["project", "list"], rules), undefined);
  });

  await t.test("allows dangerous command only with required flag", () => {
    const rules = { "terraform apply --dry-run": "allow" as const };
    assert.equal(resolveBashAction("terraform", ["apply", "--dry-run"], rules), "allow");
    assert.equal(resolveBashAction("terraform", ["apply", "-v", "--dry-run"], rules), "allow");
    assert.equal(resolveBashAction("terraform", ["apply"], rules), undefined);
    assert.equal(resolveBashAction("terraform", ["apply", "--force"], rules), undefined);
  });
...
test("resolveGlobAction", async (t) => {
  ...
  await t.test("matches patterns without path separator", () => {
    ...
    assert.equal(resolveGlobAction("src/index.ts", { "*.ts": "allow" }), undefined);
  });

  await t.test("matches patterns with **", () => {
    assert.equal(resolveGlobAction("/src/index.ts", { "**/*.ts": "allow" }), "allow");
    assert.equal(resolveGlobAction("/src/sub/index.ts", { "**/*.ts": "allow" }), "allow");
  });
